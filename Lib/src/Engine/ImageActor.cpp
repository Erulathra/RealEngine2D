...
void RealisticEngine::ImageActor::SetPosition(const Eigen::Vector2f& NewPosition)
{
    ...
    auto ResolutionPair = Engine->GetResolution();
    auto Resolution = Eigen::Vector2f(ResolutionPair->first, ResolutionPair->second);
    auto CameraLocation = Engine->CameraLocation;
    auto CameraRelativeLocation = CameraLocation - Resolution / 2.f;

    Shape->x = static_cast<int>(NewPosition[0] - Shape->w / 2.f - CameraRelativeLocation[0]);
    Shape->y = static_cast<int>(NewPosition[1] - Shape->h / 2.f - CameraRelativeLocation[1]);
}
